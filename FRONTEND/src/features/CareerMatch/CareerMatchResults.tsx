...
const Score = ({ label, value }: { label: string; value: number }) => {
  ...
  return (
    <Paper elevation={0} sx={{ p: 2.5, border: "1px solid #dfe6e1", borderRadius: 4, minWidth: 150, bgcolor: "#fff" }}>
...
const Evidence = ({ cv, requirement, rationale }: { cv: string; requirement?: string; rationale: string }) => {
  const { t } = useTranslation();
  return (
    <Box sx={{ mt: 1.5, p: 2, bgcolor: "#f4f7f5", borderRadius: 2.5, borderInlineStart: `3px solid ${colors.primary}` }}>
  ...
  return (
    <Box sx={{ my: 4 }}>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "minmax(240px, .8fr) minmax(0, 1.8fr)" }, gap: 2 }}>
        <Paper elevation={0} sx={{ p: 3, borderRadius: 5, color: "white", bgcolor: colors.primary, position: "relative", overflow: "hidden" }}>
          <Compass size={30} />
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "repeat(2, 1fr)" }, gap: 2 }}>
          {others.map((role) => (
            <Paper key={`${role.fitType}-${role.title}`} elevation={0} sx={{ p: 2.5, borderRadius: 4, border: "1px solid #dfe6e1", bgcolor: "white" }}>
              <Stack direction="row" justifyContent="space-between" gap={1} alignItems="start">
                <Chip size="small" label={t(role.fitType)} sx={{ textTransform: "capitalize", bgcolor: role.fitType === "stretch" ? "#fff1e5" : "#eaf2ed", color: role.fitType === "stretch" ? colors.amber : colors.primary, fontWeight: 800 }} />
...
export default function CareerMatchResults({ result }: { result: CareerMatchResponse }) {
  const { t } = useTranslation();
  return <Paper elevation={0} sx={{ p: { xs: 2.5, md: 5 }, borderRadius: 5, bgcolor: "white", border: "1px solid #dfe6e1" }}>
